...
import { concatAllOrdersForEventInOne, concatAllOrdersOfATableForEventInOne, concatAllOrdersOfATableInOne, getLastOrderForEvent, getLastOrderOfATable, getLastOrderOfATableForEvent } from 'src/utils/order-utils';
...
export class OrdersService {
    ...
    async createOrder( order: Order) {
        ...
        console.log(itemOrderFullName);
        let itemsEventList = "itemsEvent";
        for (const [itemId, quantity] of Object.entries(order.itemsEvent)) {
            
            itemsEventList += `|${itemId}|${quantity}`;
        }
        console.log(itemsEventList);
        const itemOrderShortName = "itemsOrder"+order.table+"|"+ new Date().toISOString();
        const itemEventShortName = "itemsEvent"+order.table+"|"+ new Date().toISOString();
        ...
        const resItemOrder : Menu = await axios.post('http://localhost:9500/menu/menus', itemOrder)
        .then(response => response.data)
        .catch(error => {
            throw new Error(`Failed to create order: ${error.message}`);
        });

        const resItemEvent: Menu = await axios.post('http://localhost:9500/menu/menus', itemEvent)  
        .then(response => response.data)
        .catch(error => {
            throw new Error(`Failed to create order: ${error.message}`);
        });

        const orderFullName = "order|"+order.table+"|"+order.event+'|'+resItemOrder._id+"|"+resItemEvent._id;
        const orderShortName = "order"+order.table+"|"+ new Date().toISOString();
        ...
        const resOrder: Menu = await axios.post('http://localhost:9500/menu/menus', orderItem)
        .then(response => response.data)
        .catch(error => {
            throw new Error(`Failed to create order: ${error.message}`);
        });
    ...
    async findAllOrdersForATableAndEvent(tableNumber: number, eventId: string) {
        ...
        const orders = concatAllOrdersOfATableForEventInOne(menuItems, tableNumber, eventId);
        return orders;
    }
    ...
    async findLastOrderOfATableAndEvent(tableNumber: number, eventId: string) {
        ...
        const orders = getLastOrderOfATableForEvent(menuItems, tableNumber, eventId);
        return orders;
    }
