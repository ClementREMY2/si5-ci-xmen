...
import { encodeObjectToBase64, findAllOrders, getLastPaymentOfTable, saveMenu } from 'src/utils/encode64-utils';
...
export class Orders64Service {
    async findAllOrdersForTable(tableNumber: number) {
        ...
        const ordersOfTable = orders.filter(order => order.table === tableNumber && order.datetime > lastPaymentOfTable.date);
    ...
    async getBillForTable(tableNumber: number) {
        ...
        const ordersOfTable: Order[] = orders.filter(order => order.table == tableNumber && (!lastPaymentOfTable || order.datetime > lastPaymentOfTable.date));
