...
export default function MenuPage() {
  ...
  const [showNames, setShowNames] = useState(false); // State to toggle name list display
  const [isRotated, setIsRotated] = useState(false); // State to toggle page rotation
  ...
  return (
    <div style={{ padding: '20px', transform: isRotated ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.5s' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: '10px', marginBottom: '20px' }}>
       ...
       {/* Display names list if showNames is true */}
       {showNames && (
        <div style={{ textAlign: 'center', marginBottom: '10px', color: 'gray' }}>
          <Typography variant="body2">Alice, Bob, Charlie, David</Typography>
        </div>
      )}
