import {
  ...
  MenuBackendNoId,
  MenuCategoryEnum,
  MenuCategoryEnumBackend,
  MenuEvent,
  MenuItem,
} from "../interfaces/Menu";
import {
  addMenu as addMenuBackend,
  getMenuById,
  getMenusGateway,
} from "../services/MenuService";

// Return a table from a backend table
const transformMenuData = (dto: MenuBackend[]): GenericMenuItem[] => {
...
export async function getOneMenuEvent(
  id: string,
  menu: MenuBackend
): Promise<MenuEvent> {
  const [type, ...ids] = menu.fullName.split("|");
  ...
  const relatedMenus = await Promise.all(
    ids.map(async (id) => {
